...
def angulo(a,b,c):
  ...
  return theta
now = datetime.now()
nombre_archivo='/home/mendel/project-posenet/datos/'+str(now)+'.txt'
angulos=open(nombre_archivo ,'a')
angulos.write('\n'+'Etiqueta  Angulo en grados')
angulos.close()
...
Puntos=(
   ...
   # ('left hip','left shoulder','right hip'),       #cadera izquierda interno
    ...
    ('left shoulder','left elbow','left hip'),      #hombro izquierdo con respecto al codo
...
def draw_pose(dwg, pose, src_size, inference_box, color='yellow', threshold=0.2):
    ...
    for a,b,c in Puntos:
        ...
        angulos.write('\n'+etiqueta+' '+str(theta))
        angulos.close()
        text=etiqueta+str(round(theta,2))
        dwg.add(dwg.text(text, insert=(int(a[0]),int(a[1])),stroke='none',fill='#900',font_size='15px',font_weight="bold",font_family="Arial"))
